trigger CaseSwarmRuleTrigger on Case_Swarm_Rule__c (before insert, before update) {

    Set<Id> csrIds = new Set<Id>();
    Map<Id, Case_Swarm_Rule__c> groupIdMap = new Map<Id, Case_Swarm_Rule__c>();
    
    //make sure everything is an ID
    for (Case_Swarm_Rule__c csr : Trigger.new) {
        try { 
